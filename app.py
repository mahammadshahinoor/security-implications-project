...
BUCKET_NAME = 'secure-file-upload-shahinoor'
...
def admin_upload():
    if request.method == "POST":
        ...
        s3.upload_fileobj(file_obj, BUCKET_NAME, encrypted_name)

        file_url = f"https://{BUCKET_NAME}.s3.amazonaws.com/{encrypted_name}"
...
def user_download(file_id):
    ...
    if request.method == "POST":
        ...
        try:
            file_url = row["encrypted_path"]
            encrypted_name = file_url.split("/")[-1]

            obj = s3.get_object(Bucket=BUCKET_NAME, Key=encrypted_name)
